...
Sub Class_Globals
	...
	Dim temperatureFromWeb As Double = 0
	Dim humidityFromWeb As Double = 0
	Dim weatherFromWeb As String = ""
	Dim airQualityFromWeb As Double = 0
...
Sub App_iconRequest
	App.Icons = Array As Int(App.get("TemperatureIconID"), App.get("HumidityIconID"), App.get("CalendarIconID"), App.get("IconIconID"), 2042,2043,2044,2045,2046,2047,2048,2049,2078,2079,2080, 487, 2)
End Sub
...
Private Sub widget_aqi(offset As Int)
	If outboundOffset(offset) Then
		Return
	End If
...
Private Sub widget_weather(offset As Int)
	...
	Else If weatherFromWeb.IndexOf("阴") >= 0 Then
		App.drawBMP(xpos,offset,App.getIcon(2049),8,8)
		App.drawBMP(xpos+5,offset,App.getIcon(2080),8,8)
	Else If weatherFromWeb.IndexOf("雾") >= 0 Then
		App.drawBMP(xpos,offset,App.getIcon(2049),8,8)
		App.drawBMP(xpos+5,offset,App.getIcon(2079),8,8)
	Else If weatherFromWeb.IndexOf("霾") >= 0 Then
		App.drawBMP(xpos,offset,App.getIcon(2049),8,8)
		App.drawBMP(xpos+5,offset,App.getIcon(2078),8,8)
	Else
...
Private Sub getTemperature As Double
	Return IIf(fahrenheit, celciusToFahrenheit(temperatureFromWeb), temperatureFromWeb)
End Sub
...
Sub App_startDownload(jobNr As Int)
	App.Download("http://aider.meizu.com/app/weather/listWeather?cityIds=101280101")
End Sub
...
Sub App_evalJobResponse(Resp As JobResponse)
	Try
		If Resp.success Then
			...
			Dim value As List = root.Get("value")
			Dim data As Map = value.Get(0)
			Dim realtime As Map = data.Get("realtime")
			temperatureFromWeb = realtime.GetDefault("temp", 0)
			humidityFromWeb = realtime.GetDefault("sD", 0)
			weatherFromWeb = realtime.GetDefault("weather", "")
			Dim pm25 As Map = data.Get("pm25")
			airQualityFromWeb = pm25.GetDefault("aqi", 0)
		End If
	Catch
